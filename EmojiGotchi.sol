// SPDX-License-Identifier: MIT

pragma solidity ^0.8.4;

import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol"; 

contract EmojiGotchi is ERC721URIStorage{

    event passTimeEvent(uint256, uint256, uint256, uint256, string);
    event feedEvent(uint256);
    event playEvent(uint256);

    constructor() ERC721("EmojiGotchi", "Your On Chain Pet") {}

    struct Token {
        uint256 happiness; 
        uint256 hunger;
        uint256 enrichment;
        uint256 lastChecked; 
        string imageURI;
    }

    uint256 tokenCounter = 0;

    mapping(address => uint256) userToTokenId;
    mapping(uint256 => Token) tokenIdToAttributes;

    string SVGBase =
        "data:image/svg+xml;base64,";

    string[] emojiBase64 = [
        "kqTwvdGV4dD48L3N2Zz4=",
        "YgTwvdGV4dD48L3N2Zz4=",
        "YkDwvdGV4dD48L3N2Zz4=",
        "YoTwvdGV4dD48L3N2Zz4=",
        "SgDwvdGV4dD48L3N2Zz4="
    ];

    function safeMint(address to) public {

        tokenCounter += 1;
        userToTokenId[to] = tokenCounter;

        _safeMint(to, tokenCounter);

        tokenIdToAttributes[tokenCounter] = Token({
            happiness: 100,
            hunger: 100,
            enrichment: 100,
            lastChecked: block.timestamp,
            imageURI: string(abi.encodePacked(SVGBase, emojiBase64[0]))
        });

        _setTokenURI(tokenCounter, tokenURI(tokenCounter));

    }

    function gotchiStats(uint256 _tokenId) public view returns(uint256, uint256, uint256, string memory) {
        return (tokenIdToAttributes[_tokenId].happiness, tokenIdToAttributes[_tokenId].hunger, tokenIdToAttributes[_tokenId].enrichment, tokenIdToAttributes[_tokenId].imageURI);
    }

    function myGotchi() public view returns(uint256, uint256, uint256, uint256, string memory) {
        uint256 tokenId = userToTokenId[msg.sender];
        return (tokenIdToAttributes[tokenId].happiness, tokenIdToAttributes[tokenId].hunger, tokenIdToAttributes[tokenId].enrichment, tokenIdToAttributes[tokenId].lastChecked, tokenIdToAttributes[tokenId].imageURI);
    }

    function passTime(uint256 _tokenId) public {
        Token memory token = tokenIdToAttributes[_tokenId];
        token.hunger -= 10;
        token.enrichment -= 10;
        token.happiness = (token.enrichment + token.happiness) / 2;
        updateURI(_tokenId);
        emit passTimeEvent(_tokenId, token.hunger, token.enrichment, token.happiness, token.imageURI);
    }

    function feed() public {
        uint256 tokenId = userToTokenId[msg.sender];
        Token memory token = tokenIdToAttributes[tokenId];
        token.hunger = 100;
        token.happiness = (token.hunger + token.enrichment) / 2;
        updateURI(tokenId);

        emit feedEvent(tokenId);
    }

    function play() public {
        uint256 tokenId = userToTokenId[msg.sender];
        Token memory token = tokenIdToAttributes[tokenId];
        token.enrichment = 100;
        token.happiness = (token.hunger + token.enrichment) / 2;
        updateURI(tokenId);

        emit playEvent(tokenId);
    }

    function updateURI(uint256 _tokenId) private {
        string memory emojiB64 = emojiBase64[0];
        if (tokenIdToAttributes[_tokenId].happiness == 100) {
            emojiB64 = emojiBase64[0];
        } else if (tokenIdToAttributes[_tokenId].happiness > 66) {
            emojiB64 = emojiBase64[1];
        } else if (tokenIdToAttributes[_tokenId].happiness > 33) {
            emojiB64 = emojiBase64[2];
        } else if (tokenIdToAttributes[_tokenId].happiness > 0) {
            emojiB64 = emojiBase64[3];
        } else if (tokenIdToAttributes[_tokenId].happiness == 0) {
            emojiB64 = emojiBase64[4];
        }
        string memory finalSVG = string(abi.encodePacked(SVGBase, emojiB64));
        tokenIdToAttributes[_tokenId].imageURI = finalSVG;
        _setTokenURI(_tokenId, tokenURI(_tokenId));
    }

    function tokenURI(uint256 _tokenId) public view override returns(string memory) {
        Token memory gotchiAttributes = tokenIdToAttributes[
            _tokenId
        ];

        string memory strHappiness = Strings.toString(
            gotchiAttributes.happiness
        );
        string memory strHunger = Strings.toString(gotchiAttributes.hunger);
        string memory strEnrichment = Strings.toString(
            gotchiAttributes.enrichment
        );

        string memory json = string(
            abi.encodePacked(
                '{"name": "Your Little Emoji Friend",',
                '"description": "Keep your pet happy!",',
                '"image": "',
                gotchiAttributes.imageURI,
                '",',
                '"traits": [',
                '{"trait_type": "Hunger","value": ',
                strHunger,
                '}, {"trait_type": "Enrichment", "value": ',
                strEnrichment,
                '}, {"trait_type": "Happiness","value": ',
                strHappiness,
                "}]",
                "}"
            )
        );

        string memory output = string(abi.encodePacked(json));
        return output;
    }

    function checkUpkeep(bytes memory checkData) public view returns (bool, bytes memory performData) {
        uint256 tokenId = userToTokenId[msg.sender];
        Token memory token = tokenIdToAttributes[tokenId];    
        if (token.happiness > 0 && token.lastChecked - block.timestamp > 60) {
            return (true, performData);
        } else {
            return (false, performData);
        }
    }

     function performUpkeep(bytes memory performData) public {
        uint256 tokenId = userToTokenId[msg.sender];
        Token memory token = tokenIdToAttributes[tokenId];   
        (bool upkeepNeeded, bytes memory data) = checkUpkeep(performData); 
        if (upkeepNeeded && token.lastChecked - block.timestamp > 60) {
            token.lastChecked = block.timestamp;
            passTime(0);
        } 
    }


}